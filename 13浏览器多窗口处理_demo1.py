...
# 第二步：根据新的句柄编号进行切换
driver.switch_to.window(new_window_handle)
# 切换句柄以后，再次获取当前的句柄编号，发现当前的句柄编号已经是新窗口的了（代表：切换窗口成功）
current_handle = driver.current_window_handle
print(f'切换后，当前所激活的窗口的句柄号：{current_handle}')

driver.find_element(By.CSS_SELECTOR, loc_support_mode).click()  # 点辅助模式
time.sleep(2)
driver.find_element(By.CSS_SELECTOR, loc_out_of_service).click()  # 退出服务
# 切回老窗口继续操作页面上的元素
# 第一步：关闭当前激活的窗口
driver.close()
time.sleep(2)
